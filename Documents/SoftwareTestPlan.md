...
## 8.6    Items Not Tested
There are two major things that we are not testing:
- Database hashing / storage of usernames (emails) and passwords: This is because we are utilizing AWS Cognito to handle these things.  Cognito is a well-known, professionally developed service that is competitively priced.  We did not feel the need to reinvent the wheel, so we'll trust that this will be a secure method that supports our primitive login flow.
- API calls: Testing API interfacing is definitely important, and we are certainly not overlooking it, but the status of API usage within this application is in limbo because of the discontinuation of the Coord API.  We're leaving the skeleton of that package in the repository in case we can port most of it over for future API use.  There are several different ways to test API calls and network performance, and we haven't decided what we'll use in the future, but there is strong potential in services like [Postman](https://www.getpostman.com/)
